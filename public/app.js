...
async function boot() {
  ...
  script.onerror = () => setStatus("PayPal failed to load. Check your PAYPAL_CLIENT_ID.");
...
async function redeemFree(rankId, player, coupon) {
  ...
  if (!freeRes.ok) {
    throw new Error(freeResult.error || "Free coupon failed.");
  }

  setStatus(freeResult.message || "Free coupon redeemed.");
}
