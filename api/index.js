...
async function sendRankByRcon(player, rankId) {
  ...
  if (!host || !password) {
    return {
      command,
      delivery: "RCON is not configured yet. Add MINECRAFT_RCON_HOST, MINECRAFT_RCON_PORT, and MINECRAFT_RCON_PASSWORD in Vercel."
    };
  }

  let rcon;
  try {
    rcon = await Rcon.connect({ host, port, password, timeout: 10000 });
    const response = await rcon.send(command);
    return {
      command,
      delivery: `Rank command sent with RCON.${response ? " Server response: " + response : ""}`
    };
  } catch (err) {
    return {
      command,
      delivery: `RCON failed: ${err.message}`
    };
  } finally {
    if (rcon) {
...
app.post("/api/redeem-free", async (req, res) => {
  try {
    ...
    const { command, delivery } = await sendRankByRcon(player, rankId);
