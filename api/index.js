...
app.post("/api/capture-order", async (req, res) => {
  try {
    ...
    if (!captureRes.ok) return res.status(500).json({ error: "PayPal capture failed.", details: capture });

    const unit = capture.purchase_units?.[0];
    const status = unit?.payments?.captures?.[0]?.status;
    if (status !== "COMPLETED") return res.status(400).json({ error: "Payment was not completed." });
