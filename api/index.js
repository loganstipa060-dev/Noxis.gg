...
function getDiscountedPrice(rank, couponCode) {
  ...
  const discounted = Math.max(0, Number(rank.price) * (1 - coupon.percentOff / 100));
...
async function paypalAccessToken() {
  ...
  if (!res.ok) throw new Error(data.error_description || "Could not connect to PayPal.");
  return data.access_token;
}

async function deliverRank(player, rankId) {
  const command = `rank set ${player} ${rankId}`;
  const endpoint = process.env.MINECRAFT_RANK_ENDPOINT;
  const secret = process.env.MINECRAFT_RANK_SECRET;

  if (!endpoint || !secret) {
    return {
      command,
      delivery: "Rank delivery endpoint is not configured yet."
    };
  }
  ...
  try {
    const res = await fetch(endpoint, {
      ...
      headers: {
        ...
        "X-Noxis-Secret": secret
      },
      body: JSON.stringify({ player, rank: rankId })
    });
    ...
    if (res.ok) {
      return { command, delivery: "Rank delivery sent to the Minecraft server." };
    }

    return { command, delivery: `Minecraft server rejected delivery. ${body}` };
  } catch (err) {
    return { command, delivery: `Minecraft server could not be reached: ${err.message}` };
  }
}
...
app.post("/api/create-order", async (req, res) => {
  try {
    ...
    if (Number(price) === 0) {
      return res.json({ free: true, player, rankId, coupon: code });
    }
    ...
    if (!orderRes.ok) return res.status(500).json({ error: "PayPal order failed.", details: order });
...
app.post("/api/redeem-free", async (req, res) => {
  try {
    ...
    const { command, delivery } = await deliverRank(player, rankId);
    res.json({ ok: true, message: `Coupon ${code} applied. ${delivery}`, command });
  } catch (err) {
